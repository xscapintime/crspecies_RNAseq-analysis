...
## save the table
save(dat, file = "motif_all.Rdata")
write.table(dat, file = "motif_all.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
# Tf family
reg <- c("MYB","^AP-1|^Atf|^ATF|^BATF","^Bach","^bHLHE","^CEBP","^CTCF","^E2F","^Egr","^ELF|^Elf","^Elk","^ETS|^Ets|^ETV|^Etv","^EWS","^FOX|^Fox","^Fra","^Gata|^GATA","^GFY","^HIF","^Hox|^HOX","^IRF\\d","^Jun","^KLF|^Klf","^Mef","^NFY","^NFkB","^Nkx","^NPAS","^NRF","^PHOX2A|^Phox2a","^PRDM","^Rfx|^RFX","^Sox","^Sp\\d|^SP\\d","^Spi|PU.1$","^Stat|^STAT","^TEAD","^USF|^Usf","^ZBTB","^ZNF|^Znf")
...
percent_mean <- (dat %>% group_by(fam) %>% summarise(mean(percent), min(P.value)) %>% arrange(`min(P.value)`, desc(`mean(percent)`)))$fam
dat$fam <- factor(dat$fam, levels = percent_mean)
...
tf_order <- (dat %>% group_by(fam, group) %>% arrange(desc(percent), P.value, .by_group = T))$ortholog %>% unique()
...
ggsave("motif_dotplot_alltype.pdf", width = 20, height = 2.8)
