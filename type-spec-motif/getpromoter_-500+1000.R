...
## get promoter fasta
# for (i in 1:6) {
#     prom_seq <- rush_promo(db = src, upstream = 1000, downstream = 500, genes = genelist[[i]], sequence = T)
...
## get promoter bed
for (i in 1:6) {
    prom <- rush_promo(db = src, upstream = 1000, downstream = 500, genes = genelist[[i]], sequence = F)
    rtracklayer::export.bed(unlist(prom), paste0("prom_bed/", names(genelist)[i], "_prom_500-1k.bed"))
}
