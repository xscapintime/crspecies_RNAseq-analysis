...
# position score
load("position_score_t3.Rdata")


# BH p-val
load("manova_bh_adjpval_t3.Rdata")
...
write.table(dat, file = "manova-position_type3.txt", quote = F, sep = "\t")
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    labs(x = "Diapause / E4.5", y = "Type Ⅲ Arrested / E4") +
    xlim(-1, 1) + ylim(-1, 1) +
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_t3.png")
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_corr_t3.png")
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_anti_t3.png")
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    geom_text_repel(data = dat %>%
                filter(abs(dat$mouse_ps * dat$human_ps) > 0.12),
                mapping = aes(label = dat %>%
                                filter(abs(dat$mouse_ps * dat$human_ps) > 0.12) %>%
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_seltext_t3.png")
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_diapause_t3.png")