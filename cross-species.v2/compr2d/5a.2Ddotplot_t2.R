...
# position score
load("position_score_t2.Rdata")


# BH p-val
load("manova_bh_adjpval_t2.Rdata")
...
dat$group <- ifelse(dat$mouse_ps * dat$human_ps > 0,
                    ifelse(abs(dat$mouse_ps) >= 0.05 & abs(dat$human_ps) >= 0.05, "corr", "other"),
                    ifelse(abs(dat$mouse_ps) >= 0.05 & abs(dat$human_ps) >= 0.05, "anti", "other"))
...
write.table(dat, file = "manova-position_type2.txt", quote = F, sep = "\t")
...
p <- ggplot(dat %>% filter(bh_adjpval < 0.1),
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    labs(x = "Diapause / E4.5", y = "Type Ⅱ Arrested / Morula") +
    xlim(-1, 1) + ylim(-1, 1) +
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_t2.pdf")
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    geom_text_repel(data = dat %>%
                filter(group == "corr" & bh_adjpval < 0.1),
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_corr_t2.png")
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    geom_text_repel(data = dat %>%
                filter(group == "anti" & bh_adjpval < 0.1),
                mapping = aes(label = dat %>%
                                filter(group == "anti" & bh_adjpval < 0.1) %>%
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_anti_t2.png")
...
p + geom_point(alpha = .6, aes(color = group, size = bh_adjpval,
    ...
    geom_text_repel(data = dat %>%
                filter(abs(dat$mouse_ps * dat$human_ps) > 0.1),
                mapping = aes(label = dat %>%
                                filter(abs(dat$mouse_ps * dat$human_ps) > 0.1) %>%
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_allpath_seltext_t2.png")
...
ggsave(width = 7.6, height = 7.6, filename = "../figs/2d_diapause_t2.png")