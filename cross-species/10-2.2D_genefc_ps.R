# 2D plot based on gene FC
# position socre from 9-2
# GO BP for a look
# ------------------------
...
human_bp <- read.csv("human_bp_gene_ps.txt", sep = "\t", header = T)
mouse_bp <- read.csv("mouse_bp_gene_ps.txt", sep = "\t", header = T)
...
dat$group <- ifelse(dat$ps.x * dat$ps.y > 0,
                    ifelse(abs(dat$ps.x) >= 0.2 & abs(dat$ps.y) >= 0.2, "corr", "other"),
                    ifelse(abs(dat$ps.x) >= 0.2 & abs(dat$ps.y) >= 0.2, "anti", "other"))
